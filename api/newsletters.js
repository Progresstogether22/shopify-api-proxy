...
export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `https://${dc}.api.mailchimp.com/3.0/campaigns?status=sent&count=${count}&sort_field=send_time&sort_dir=DESC&since_send_time=${encodeURIComponent(sinceStr)}&fields=campaigns.id,campaigns.settings.title,campaigns.settings.subject_line,campaigns.settings.preview_text,campaigns.send_time,campaigns.archive_url,campaigns.emails_sent,campaigns.tags,campaigns.recipients.segment_text`;
