...
export default async function handler(req, res) {
  ...
  try {
    ...
    const query = `SELECT Membership_Start_Date__c, Next_Renewal_Date__c, Renewal_Month__c FROM Contact WHERE Email = '${email.replace(/'/g, "\\'")}' LIMIT 1`;
    ...
    return res.status(200).json({
      ...
      renewal_month: contact.Renewal_Month__c,
    });
  } catch (err) {
